...
Image::Image(ivec2 size, i32 num_components, u32 color, bool warning)
{
    ...
    if (!num_bytes)
    {
        ...
        if (warning)
            DV_LOG->writef_warning("{} | !num_bytes", DV_FUNCSIG);
    }
    else
    {
        size_ = size;
...
Image::Image(const StrUtf8& file_path, bool use_error_image)
{
    ...
    data_ = stbi_load(file_path.c_str(), &size_.x, &size_.y, &num_components_, 0);

    if (!data_)
    {
        DV_LOG->writef_error("{} | {} | {}", DV_FUNCSIG, file_path, stbi_failure_reason());
...
void Image::save_png(const StrUtf8& path) const
{
...
Image Image::to_rgba(u32 color)
{
    ...
    if (num_components() != 1)
    {
        DV_LOG->writef_error("{} | num_components() != 1", DV_FUNCSIG);
        return *this;
    }
