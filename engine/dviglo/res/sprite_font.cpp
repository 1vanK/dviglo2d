...
SpriteFont::SpriteFont(const StrUtf8& file_path)
{
    ...
    for (xml_node char_node : root_node.child("chars"))
    {
        ...
        glyph.offset_x = (i16)char_node.attribute("xoffset").as_uint(); // TODO: Переименовать в offset_x
...
void SpriteFont::save(const StrUtf8& file_path)
{
    ...
    for (Texture* texture : textures_)
    {
        if (!texture->image())
        {
            DV_LOG->write_error(format("SpriteFont::save(\"{}\") | !texture->image()", file_path));
            return;
        }
    }
    ...
    {
        DV_LOG->write_error(format("SpriteFont::save(\"{}\") | ext != \"fnt\"", file_path));
        return;
    }
    ...
    for (c32 code_point : code_points)
    {
        ...
        char_node.append_attribute("x") = glyph.x;
        char_node.append_attribute("y") = glyph.y;
        char_node.append_attribute("width") = glyph.width;
        char_node.append_attribute("height") = glyph.height;
        char_node.append_attribute("xoffset") = glyph.offset_x; // TODO: Переименовать в offset_x
        char_node.append_attribute("yoffset") = glyph.offset_y;
