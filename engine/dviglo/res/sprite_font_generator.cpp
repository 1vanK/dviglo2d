...
SpriteFont::SpriteFont(const SFSettingsSimple& settings, i64* generation_time_ms)
{
    ...
    while (glyph_index != 0)
    {
        ...
        rendered_glyph.advance_x = round_to_pixels(face.get()->glyph->metrics.horiAdvance);
...
SpriteFont::SpriteFont(const SFSettingsContour& settings, i64* generation_time_ms)
{
    ...
    DV_LOG->writef_info("{} | {} | Generated in {} ms", DV_FUNCSIG, settings.src_path, duration_ms);
}
