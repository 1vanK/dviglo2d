...
// В случае ошибки возвращает символ '?' и прерывает декодирование.
// Использование:
// size_t offset = 0;
// while (offset < str.length()) { c32 code_point = next_code_point(str, offset); }
constexpr c32 next_code_point(StrViewUtf8 str, size_t& inout_offset)
{
    if (inout_offset >= str.length())
        return '?';

    // Получаем байт из строки и после этого инкрементируем смещение
    u8 byte1 = str[inout_offset++];
    ...
    {
        if (inout_offset == str.length()) // Проверяем, что второй байт вообще есть (offset уже указывает на очередной байт)
            goto error;

        // Получаем второй байт последовательности
        u8 byte2 = str[inout_offset++];
        ...
        // 0b00111111u имеет тип u32, поэтому тут всё автоматически преобразуется к c32
    ...
    {
        if (inout_offset + 1 >= str.length())
            goto error;

        u8 byte2 = str[inout_offset++];
        u8 byte3 = str[inout_offset++];
    ...
    {
        if (inout_offset + 2 >= str.length())
            goto error;

        u8 byte2 = str[inout_offset++];
        u8 byte3 = str[inout_offset++];
        u8 byte4 = str[inout_offset++];
...
constexpr StrUtf8 to_utf8(std::vector<c32> code_points)
{
    StrUtf8 ret;
...
constexpr bool starts_with(const StrUtf8& str, const StrUtf8& prefix)
{
    if (prefix.empty())
        return true;
